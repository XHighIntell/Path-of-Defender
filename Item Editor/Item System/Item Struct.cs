...
namespace Path_of_Defender {
    ...
    public struct Mod {
        ...
        public ModType Mod_Type { get { return Type; } set { Type = value; } }
        ...
        public override string ToString() {
            return ToString(Type, Value1, Value2);
                if (Type == ModType.Strength) { return "+" + Value1 + " to Strength"; }
                else if (Type == ModType.Dexterity) { return "+" + Value1 + " to Dexterity"; }
                else if (Type == ModType.Intelligence) { return "+" + Value1 + " to Intelligence"; }
                else if (Type == ModType.Attributes) { return "+" + Value1 + " to all Attributes"; }

                else if (Type == ModType.Life) { return "+" + Value1 + " to maximum Life"; }
                else if (Type == ModType.Increased_Life) { return Math.Round(Value1 * 100).ToString() + "% increased maximum Life"; }

                else if (Type == ModType.Mana) { return "+" + Value1 + " to maximum Mana"; }
                else if (Type == ModType.Increased_Mana) { return Math.Round(Value1 * 100).ToString() + "% increased maximum Mana"; }

                else if (Type == ModType.Fire_Resistance) { return "+" + Math.Round(Value1 * 100).ToString() + "% to Fire Resistance"; }
                else if (Type == ModType.Cold_Resistance) { return "+" + Math.Round(Value1 * 100).ToString() + "% to Cold Resistance"; }
                else if (Type == ModType.Lightning_Resistance) { return "+" + Math.Round(Value1 * 100).ToString() + "% to Lightning Resistance"; }
                else if (Type == ModType.Chaos_Resistance) { return "+" + Math.Round(Value1 * 100).ToString() + "% to Chaos Resistance"; }

                else if (Type == ModType.Increased_Attack_Speed) { return Math.Round(Value1 * 100).ToString() + "% increased Attack Speed"; }
                else if (Type == ModType.Increased_Cast_Speed) { return Math.Round(Value1 * 100).ToString() + "% increased Cast Speed"; }
                else if (Type == ModType.Increased_Duration) { return Math.Round(Value1 * 100).ToString() + "% increased Duration"; }
                else if (Type == ModType.Increased_Buff_Duration) { return Math.Round(Value1 * 100).ToString() + "% increased Buff Duration"; }
                else if (Type == ModType.Increased_AOE) { return Math.Round(Value1 * 100).ToString() + "% increased Radius of Area Skills"; }
    ...
    public struct ItemStructure {
        ...
        public static void Save(ItemStructure item, string file) {
            FileSystem.FileOpen(4, file, OpenMode.Binary);
            FileSystem.FilePut(4, item);
            FileSystem.FileClose(4);
        }
        public static ItemStructure Load(string file) {
            ValueType tmp = new ItemStructure();
            FileSystem.FileOpen(4, file, OpenMode.Binary);
            FileSystem.FileGet(4, ref tmp);
            FileSystem.FileClose(4);

            return (ItemStructure)tmp;
        }
    }
    ...
    [TypeConverter(typeof(ItemCostConverter))]
    public struct ItemCost {
        ...
        public string item { get { return Item_Base_Name; } set { Item_Base_Name = value; } }
