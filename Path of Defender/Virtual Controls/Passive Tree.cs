...
namespace Path_of_Defender {
    ...
    public partial class PassiveTree : VirtualControl {
        ...
        public int HitTestLine(float x, float y) {
            ...
            //    point = PointRotate(new Vector2(xx, yy), -Lines[i].Alpha, SelectedData.Lines[i].Position);
            //    x = point.X; y = point.Y;
            //    if (Lines[i].Style == LineStyle.Line) { connectBase = PSS.Connect.Line; }
            //    else if (Lines[i].Style == LineStyle.Tiny_Circle) { connectBase = PSS.Connect.TinyCircle; }
            //    else if (Lines[i].Style == LineStyle.Small_Circle) { connectBase = PSS.Connect.SmallCircle; }
            //    else if (Lines[i].Style == LineStyle.Medium_Circle) { connectBase = PSS.Connect.MediumCircle; }
            //    else { connectBase = PSS.Connect.LargeCircle; }


            //    if (Lines[i].Position.X - connectBase.Origin.X < x && x < SelectedData.Lines[i].Position.X - connectBase.Origin.X + SelectedData.Lines[i].Width &&
            //        SelectedData.Lines[i].Position.Y - connectBase.Origin.Y < y && y < SelectedData.Lines[i].Position.Y - connectBase.Origin.Y + connectBase.Rect.Height) { return i; }
            //}
        ...
        float[] Zooms = { 0.25f, 0.5f, 0.75f, 1 };
        public void ZoomIn() {
            float current = Scale;
            //if (current > 0.1f) { current = current - 0.1f; }
            //current = (float)(Math.Round(current * 100) / 100);

            int Index = Array.IndexOf<float>(Zooms, Scale);
            if (Index > 0) { current = Zooms[Index - 1]; }
            
            if (GameSetting.Width <= SelectedData.Rect.Width * current && GameSetting.Height < SelectedData.Rect.Height * current) {
                Vector2 center = GetCenterPoint();
                Scale = current;
                ViewAsCenter(center);
            }
        ...
        public void ZoomOut() {
            ...
            int Index = Array.IndexOf<float>(Zooms, Scale);
            if (Index < Zooms.Length - 1) { current = Zooms[Index + 1]; }

            Vector2 center = GetCenterPoint();
            if (current > 1) { Scale = 1; }
            else { Scale = current; }
            ViewAsCenter(center);
        }
        #endregion

        Vector2 MouseMove_PointF; int Hover; Vector2 Hit;
        bool CanMovePosition; //Khi = true thì MouseMove có thề di chuyển PassiveTree
        int Hit_Skill_Index;
        ...
        private void Passive_Editor_ItemMouseClick(object sender, MouseEventArgs e) {
            if (e.Button == MouseButtons.Left) {
                SelectedData.Skills[e.Clicks].Status = SkillStatus.Allocated; RefreshEditor();
            } else if (e.Button == MouseButtons.Right) {
                SelectedData.Skills[e.Clicks].Status = SkillStatus.Unallocated; RefreshEditor();
            }
        }
        private void Passive_Editor_MouseWheel(object sender, MouseEventArgs e) { 
            if (e.Delta < 0) {
                if (Scale <= 0.1f && Scale > 0.01f) { Zoom(Scale - 0.01f); } 
                else if (Scale > 0.1f) { Zoom(Scale - 0.1f); }
            } else {
                if (0.00f <= Scale && Scale <= 0.09f) { Zoom(Scale + 0.01f); }
                else if (Scale >= 0.1f) { Zoom(Scale + 0.1f); }
            }
        ...
        public void DrawEditor(bool drawSkillNumber, bool drawSkills, bool drawLines, bool drawLabes, bool drawBackground) {
            ...
            e.SpriteBatch.Begin(SpriteSortMode.Deferred, null, e.SpriteBatch.GraphicsDevice.SamplerStates.PointWrap);
    ...
    public class PassiveTreeData {
        ...
        public bool CanUnallocated(int Index) {
            bool Return;
            if (Skills[Index].Status == SkillStatus.Allocated) {
                Skills[Index].Status = SkillStatus.Unallocated;
                for (int i = 0; i < Skills[Index].ConnectedSkills.Length; i++) {
                    //if (Skills[Index].ConnectedSkills[i] == 0) { return true; }

                    //nếu skill kết nối Allocated thì mới cần kiểm tra
                    if (Skills[Index].ConnectedSkills[i] != 0 && Skills[Skills[Index].ConnectedSkills[i]].Status == SkillStatus.Allocated) { 
                        Return = CanGoTo(Skills[Index].ConnectedSkills[i], 0);
                        if (Return == false) {
                            Skills[Index].Status = SkillStatus.Allocated;
                            return Return;
                        }
                    }
                }
        ...
        public bool CanGoTo(int SrcIndex, int DesIndex, int CantGo = -1) {
            bool Return;
            for (int i = 0; i < Skills[SrcIndex].ConnectedSkills.Length; i++) {
                if (Skills[SrcIndex].ConnectedSkills[i] != CantGo) { 
                    if (Skills[SrcIndex].ConnectedSkills[i] == DesIndex) {
                        return true;
                    }
                    if (Skills[Skills[SrcIndex].ConnectedSkills[i]].Status == SkillStatus.Allocated) {
                        Return = CanGoTo(Skills[SrcIndex].ConnectedSkills[i], DesIndex, SrcIndex);
    ...
    public struct PassiveSkill {
        ...
        public void DrawBackground(SpriteBatch spriteBatch, float Scale, Vector2 position) {
            ...
            if (Status == SkillStatus.CanAllocated || Status == SkillStatus.Unallocated) { color = 0.5f; }
